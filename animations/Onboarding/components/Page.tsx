...
const Page: React.FC<PageProps> = ({ page, translateX, index }) => {
  ...
  const rImageStyle = useAnimatedStyle(() => {
    const progress = interpolate(translateX.value, inputRange, [0, 0, 1], Extrapolate.CLAMP);

    const opacity = interpolate(translateX.value, inputRange, [0.5, 1, 0.5], Extrapolate.CLAMP);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.circleContainer}>
        ...
        <Animated.Image source={page.source} style={[styles.image, rImageStyle]} resizeMode={"contain"} />
      </View>
