...
// Local imports
import { CANVAS_HEIGHT, CANVAS_WIDTH, MAX_DISTANCE } from "../constants/constants";

// interface for RoundedItemProps
...
// Memoized component
const RoundedItem: React.FC<RoundedItemProps> = React.memo(({ point, progress, ...squareProps }) => {
  ...
  const distance = useComputedValue(() => {
    ...
    previousDistance.current = Math.sqrt((point.current.x - x) ** 2 + (point.current.y - y) ** 2);
    return previousDistance.current;
  }, [point]);

  const scale = useComputedValue(() => {
    return interpolate(distance.current * progress.current, [0, MAX_DISTANCE / 2], [1, 0], {
